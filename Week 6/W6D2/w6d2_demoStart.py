...
with open("Week 6/W6D2/party.csv", encoding="utf-8") as csvfile:
...
while ans == "y":
    ...
    if search_type not in ["1", "2", "3", "4"]:
        ...
        #show all values in class_type of either dragon or elf --> sequential
        ...
        #below is sorting for ascending alphabetical order
        for i in range(len(name) - 1):
            for j in range(len(name) - 1):
                if name[j] > name[j + 1]: #< if descending
        ...
        #check for bubble sort -- sorting in ascending order by name
        display("x", len(name))

        #binary search: must be performed on ordered or sorted lists populated with unique values - can only find one item or value

        min = 0                     #lowest index
        max = len(name) - 1         #highest index
        mid = int((min + max) / 2)  #middle index

        while min < max and search.lower() != name[mid].lower(): #> if descending
